...
func InitialModel() model {
	return model{
		currentState: Menu,
		menuItems:    []string{"Start Game", "Difficulty", "Quit"},
		difficulty:   0.30,
		cursorPosition: 0,
		board: board.NewBoard(50, 50, 0.30),
	}
}
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case Menu:
		...
		case tea.KeyMsg:
			...
			case "down":
				m.cursorPosition++
				if m.cursorPosition >= len(m.menuItems) {
					m.cursorPosition = 0
				}
			case "up":
				m.cursorPosition--
				if m.cursorPosition < 0 {
					m.cursorPosition = len(m.menuItems) - 1
				}
			case "enter":
				switch m.menuItems[m.cursorPosition] {
				case "Quit":
					return m, tea.Quit
				case "Start Game":
					m.board = board.NewBoard(50, 50, m.difficulty)
					m.currentState = Game
				case "Difficulty":
					m.difficulty += 0.1
					if m.difficulty > 0.9 {
...
func (m model) View() string {
	...
	case Menu:
		...
		s += "Use up/down arrows to navigate, enter to select.\n\n"
		...
		for i, item := range m.menuItems {
			...
			if item == "Difficulty" {
				item += " (" + strconv.FormatFloat(m.difficulty, 'f', 2, 64) + ")"
			}
			s += cursor + " " + item + "\n"
		}
